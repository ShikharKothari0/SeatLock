...
Day 24 targets: reduce hold endpoint p99 below 1200ms and seat listing p99 around 1000ms through HikariCP pool and Lettuce connection tuning.



**NOTE: The** **improvements may seem a bit under-whelming but it is to be considered that the whole application along with all the services are being run on the host machine at the same time and not on a dedicated server.**
...
## **Day 24 Post-tuning Results**



Changes applied: - HikariCP maximum-pool-size: 10 → 50 - Lettuceconnection pool enabled: max-active=20 - JVM:-XX:MaxGCPauseMillis=100 -Xms512m -Xmx1024m
...
|Endpoint|p99 before|p99 after|difference in p99|
|-|-|-|-|
|GET /api/events/{eventId}/seats|1220ms|1014ms|-206ms|
|POST /api/seats/{seatId}/hold|1540ms|1178|362ms|
...
|-|-|-|-|
|Total requests|139406|144127|+4721 (+3.39%)|
|Mean response time|337ms|308ms|-29ms (-8.6%)|
|p75|554ms|527ms|-27ms (-4.9%)|
|p95|774ms|757ms|-17ms(-2.2%)|
|Maximum response time|2147ms|1497ms|=-650ms(-30.3%)|
|Mean Throughput|\~900 req/s|\~1022 req/s|+13.6%|
